...
//
//  Added acceleration modification routine to main loop, disabled (or
//  redirected to stdout) most diagnostic output.
//
...
void run() {
    ...
    do {      /* main loop */
        ...
        if((All.Time-All.TimeOfFirstSnapshot)>=0) {/* check whether it's time for a snapshot file */
            ...
            else if(All.ComovingIntegrationOn)
        ...
        // MODIFICATION
        // modify acceleration for particle interaction before advancing particle
        modify_accel();
...
//  fprintf(stdout,"Timestep %d, Time: %g\n",All.NumCurrentTiStep,All.Time);
...
//  fprintf(stdout,"%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n",
//	  All.Time,
//	  SysState.EnergyInt,
//	  SysState.EnergyPot,
//	  SysState.EnergyKin,
